...
search_queries = ['cat:cs.CL','cat:cs.DC','cat:cs.IR','cat:cs.CY']  # Modify or add more subcategories as per your requirement
max_results_per_query = 2 # Maximum number of papers to download for each subcategory
num_papers_to_download = 5 # Number of papers to download randomly
...
for search_query in search_queries:
    query = f"submittedDate:[{today.year:04d}-{today.month:02d}-{today.day:02d} TO {today.year:04d}-{today.month:02d}-{today.day:02d}] {search_query}"  # Add lang:en to filter English papers
...
# Randomly select 5 papers from the collected URLs
...
for i, pdf_file in enumerate(os.listdir(folder)):
    if pdf_file.endswith(".pdf"):
      !java -Xmx6g -jar science-parse-cli-assembly-2.0.3.jar /arxiv -o ./json
